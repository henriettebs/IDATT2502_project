...
def get_time_interval():
    ...
    date_end = (dt.date.today() - dt.timedelta(days=900)).strftime('%Y-%m-%d')
...
def main():
    ...
    scaled_lstm,lstm_history =  lstm_main(raw_seq,1,1,False)
    descaled_lstm = []
    for avg in scaled_lstm:
        descaled_lstm.append(scaler.inverse_transform(np.array(avg).reshape(-1,1))[0][0])
    print("done with first \n")
    
    scaled_lstm_attention,lstm_attention_history =  lstm_main(raw_seq,1,1,True)
    ...
    scaled_bi_lstm,bi_lstm_history =  bi_lstm_main(raw_seq,1,1,False)
    descaled_bi_lstm = []
    ...
    scaled_bi_lstm_attention,bi_lstm_attention_history =  bi_lstm_main(raw_seq,1,1,True)
    ...
    # writeFile = open("msft_values.txt", "w")
